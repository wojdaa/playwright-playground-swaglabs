...
test.describe('Shopping Cart Management', () => {
  ...
  test('Add Single Item to Cart from Inventory', async () => {
  ...
  test('Add Multiple Items to Cart', async () => {
    ...
    await inventoryPage.verifyProductButtonState('Sauce Labs Backpack', 'remove');
    await inventoryPage.verifyProductButtonState('Sauce Labs Bike Light', 'remove');
    await inventoryPage.verifyProductButtonState('Sauce Labs Onesie', 'remove');
  });

  test('Remove Item from Cart on Inventory Page', async () => {
    await inventoryPage.addProductToCart('Sauce Labs Backpack');
    await inventoryPage.verifyCartBadgeCount(1);
